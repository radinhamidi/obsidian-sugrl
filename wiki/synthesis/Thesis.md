...
- **Pre-emption risk (medium, verification pending):** [[MHVGCL]] (Wu et al., Applied Soft Computing Jan 2025) is a candidate closest ancestor — from its **abstract**, the loss family looks like InfoNCE over same-node-across-hops with no augmentation. **PDF not yet obtained** (paywalled ScienceDirect, no arxiv preprint, ResearchGate blocked); the architectural differentiator paragraph below is inferred from abstract phrasing and MUST be audited against the method section before it appears in the paper. Tentative three-way differentiator (pending verification): (1) D6c precomputes `Â^k X` once; MHVGCL's abstract implies per-epoch propagation of an MLP-encoded base view, (2) D6c has per-depth W_k residual projection; MHVGCL's abstract implies a single shared output head, (3) D6c contrasts raw `Â^k X`; MHVGCL appears to contrast encoder outputs. Also adjacent: [[MVGRL]] (adjacency vs. PPR, 2 views, GNN encoder — verified from PDF) and [[DGD]] (decoupled-GCN + BCE group discrimination — abstract-level reading only, also unverified). **Action before paper submission:** obtain MHVGCL + DGD PDFs and re-audit.
