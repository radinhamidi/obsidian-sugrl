...
- **Pre-emption risk (medium):** [[MHVGCL]] (Wu et al., Applied Soft Computing Jan 2025) is the closest known ancestor — **same loss family** (InfoNCE over same-node-across-hops, no augmentation), but architecturally apart from D6c: MHVGCL applies MLP **before** propagation (`Â^k · MLP(X)`, APPNP order, per-epoch), uses a **single shared head** across hops, and pays per-epoch GNN cost. D6c propagates **raw X** at load time once, uses **per-depth W_k** with residual, and runs at MLP-speed. **Three-way differentiator for the paper:** (1) precompute vs. per-epoch cost, (2) per-depth W_k vs. shared head (D6a/D6b ablations show residual per-depth is load-bearing; MHVGCL's shared head is spiritually our D1 which hard-failed), (3) contrast on raw `Â^k X` surfaces the simplex-collapse signal directly. Also adjacent: [[MVGRL]] (adjacency vs. PPR, 2 views, GNN encoder) and [[DGD]] (decoupled-GCN + BCE group discrimination, aggregates hops instead of contrasting them — low risk).
