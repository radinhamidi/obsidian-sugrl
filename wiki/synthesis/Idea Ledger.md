...
- **Reviewer attacks identified (literature audit 2026-04-24):**
  1. **"This is MHVGCL (Wu et al., Applied Soft Computing 2025)."** Candidate pre-emption — surfaced 2026-04-24 pass-2 audit. **⚠ PDF not yet obtained; all architectural claims below are from abstract paraphrases only and must be re-audited against the method section before relying on this defense.** Tentative three-way differentiator (pending verification): (a) **precompute vs. per-epoch** — abstract implies MHVGCL runs `Â^k · MLP(X)` per epoch; D6c's `Â^k X` is precomputed once (MLP-speed). (b) **per-depth W_k vs. shared head** — abstract implies single shared head across hops, which would be architecturally equivalent to our D1 (INQ-2026-04-23-001) that hard-failed under homophily + WD. D6c's per-depth residual `Z_k = X_k + W_k X_k` is load-bearing per our own ablations (D6a/D6b without residual fail Computers by −5.14/−7.21). (c) **raw X vs. encoder output** — abstract implies MHVGCL contrasts encoder-outputs; D6c contrasts raw `Â^k X` directly. See [[MHVGCL]] for the unverified-claim notice. **Action:** obtain PDF before paper submission; falsify or confirm the three differentiators.
  ...
  3. "This is DGD (Neurocomputing 2024) — decoupled-GCN + SSL on precomputed features." **⚠ DGD PDF also not obtained.** Tentative defense from abstract: DGD uses BCE group discrimination (inherited from [[GGD]]), aggregates hops into a single embedding before the loss; D6c keeps hops separate as contrast views under InfoNCE. Risk tentatively LOW, but must be verified. See [[DGD]].
