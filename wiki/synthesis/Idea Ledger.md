...
- **Reviewer attacks identified (literature audit 2026-04-24):**
  1. **"This is MHVGCL (Wu et al., Applied Soft Computing 2025)."** HIGHEST-RISK pre-emption — surfaced in 2026-04-24 pass-2 audit. MHVGCL uses same-node-across-hops InfoNCE-family loss with no augmentation. Three-way differentiator for the paper: (a) **precompute vs. per-epoch** — MHVGCL's `Â^k · MLP(X)` runs per epoch (GNN-encoder cost); D6c's `Â^k X` precomputed once (MLP-speed). (b) **per-depth W_k vs. shared head** — MHVGCL's single shared head across hops is architecturally equivalent to our D1 (INQ-2026-04-23-001), which hard-failed under homophily + WD (shared-head W_k collapse). D6c's per-depth residual `Z_k = X_k + W_k X_k` is load-bearing (D6a/D6b without residual fail Computers by −5.14/−7.21). (c) **raw X vs. encoder output** — D6c contrasts raw `Â^k X` directly, surfacing the simplex-collapse signal from Ji et al. 2025; MHVGCL contrasts encoder-outputs, mixing encoder inductive bias with hop signal. See [[MHVGCL]]. **Action:** ingest PDF, verify loss form, benchmark if they report arxiv.
  2. "This is MVGRL (Hassani & Khasahmadi 2020, arXiv 2006.05582) with depth instead of diffusion." Defense: MVGRL uses 2 fixed structural views (adjacency + PPR), both passed through a GNN encoder per epoch; D6c uses K precomputed depths, no encoder. Demoted from "closest ancestor" after MHVGCL surfaced.
  3. "This is DGD (Neurocomputing 2024) — decoupled-GCN + SSL on precomputed features." Defense: DGD uses BCE group discrimination (inherited from [[GGD]]), aggregates hops into a single embedding before the loss; D6c keeps hops separate as contrast views under InfoNCE. DGD does not use hops as a contrast axis. See [[DGD]]. **LOW risk.**
