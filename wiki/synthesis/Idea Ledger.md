...
- **Reviewer attacks identified (literature audit 2026-04-24, still ongoing):**
  1. "This is MVGRL (Hassani & Khasahmadi 2020, arXiv 2006.05582) with depth instead of diffusion." Defense: MVGRL uses 2 fixed structural views (adjacency + PPR), both passed through a GNN encoder per epoch; D6c uses K precomputed depths, no encoder, per-epoch cost is one linear projection per depth. Architectural category apart. Still need sharper differentiator on the "cross-view of same graph" line.
  2. "PolyGCL already does multi-scale spectral contrast." Defense: PolyGCL uses 2 views (low-pass + high-pass Chebyshev), spectral GNN encoder per view; D6c uses K monotone-low-pass views at different depths, no encoder.
  3. "Graph MAE-style methods already cover pre-training at precompute-cheap." Defense: MAE methods require a full GNN encoder + decoder per epoch; D6c has neither.
  - **Still need audit on:** "multi-scale contrastive", "hop-level contrastive", "SGC-contrastive", "decoupled propagation SSL." No direct "Â^k vs Â^{k'} InfoNCE at precompute" paper surfaced in HF paper_search top-24 results on three queries.
