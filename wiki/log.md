...
## [2026-04-24] ingest | MHVGCL + DGD PDFs verified; pre-emption defense sharpened
Radin added both PDFs to raw/papers/. Extracted via pdftotext -layout. Full method-section audit complete; UNVERIFIED flags removed. **MHVGCL findings vs. my abstract-level read:** (1) ✅ InfoNCE loss, same-node-across-views positives, cross-node negatives — confirmed (Eq. 7). (2) ✅ Single shared W (base view) — confirmed. (3) ✅ Per-epoch propagation — confirmed (H^(0) depends on trainable W). (4) ❌ Propagation is NOT `Â^k · MLP(X)` as I had claimed — the actual formula is `H^(k) = ELU(α·Â·H^(k-1) + (1−α)·H^(k-1))` (APPNP-style iterative fusion with ELU nonlinearity), different from raw Â^k powers. (5) ⚠ Missed: MHVGCL-MV readout is **concat of all K+1 views** — structurally identical to D6c's Z_concat default. This is a closer overlap than flagged initially. Pre-emption risk confirmed at MEDIUM with sharpened three-differentiator defense now in [[Thesis]] and [[Idea Ledger]]. **Scope gap D6c exploits:** MHVGCL does NOT report ogbn-arxiv and uses limited-label regime (1–4 labels/class); our +8.05 arxiv result is not pre-empted. **DGD findings:** (1) ✅ BCE group-discrimination loss — confirmed (Eq. 2). (2) ✅ Propagate-then-MLP order — confirmed. (3) ✅ Partial corruption negative sampling — confirmed. (4) ❌ "Aggregates hops" was imprecise — DGD uses a SINGLE fixed k at training, with an m=10 post-hoc A^m H_θ skip at inference, not multi-hop aggregation. Contrast axis is real-node-vs-corrupted-node, not depth-vs-depth. LOW risk confirmed. Updated [[MHVGCL]] and [[DGD]] entity pages with full verified method sections + correct equations + scope tables. Updated [[Thesis]] § Known risks to drop "verification pending" language. Updated [[Idea Ledger]] D6c reviewer-attacks 1 and 3 with verified differentiators. INQ-008 draft (phase-2 datasets + efficiency benchmark) still uncommitted awaiting Radin's ship/revise/hold call.

## [2026-04-24] inquiry-open | INQ-2026-04-24-001 D6c Phase-2 datasets + efficiency benchmark
Filed [[INQ-2026-04-24-001]] to CA after Radin's go-ahead on slate items 3+4 from the 2026-04-24 INQ-007 triage. Two primary configs in parallel: **Config A** (D6c on CiteSeer, PubMed, Amazon-Photo, Coauthor-CS, 5-seed × 5-restart, report Z_mean + Z_concat + per-depth Z_k + W_k norms + cos(W_k,W_k') — expands headline table from 3 to 7 homophilic benchmarks) and **Config B** (efficiency benchmark on Cora/Computers/ogbn-arxiv: per-epoch wall-clock, total-to-convergence, peak GPU memory, final accuracy for D6c + SUGRL + GGD + BGRL + GraphMAE + PolyGCL + MHVGCL-if-implementable on matched hardware — supports Pareto figure and verified MHVGCL pre-emption defense). Optional Configs C (K_SET ablation) and D (τ_c sweep) if capacity allows. Pass bar per Phase-2 dataset: max(Z_mean, Z_concat) > raw best-single-depth, seed-matched 4/5. Constraint non-propagation reminders embedded: WD-on-W_k required (Config D of INQ-007 ruled out V-WD), Z_concat is default readout after INQ-007 arxiv +3.43 gap, no α-routing (INQ-007 Config C closed it). Priority high, not blocking RA wiki continuation.

## [2026-04-24] inquiry-answer | INQ-2026-04-24-001 D6c Phase-2 + efficiency benchmark
CA ran Config A (D6c on CiteSeer, PubMed, Photo, CS; 5 seeds × 5 probe restarts per dataset) and Config B (efficiency benchmark: D6c + SUGRL on Cora, Computers, ogbn-arxiv matched on Vector A40-48GB). Full numbers + tables under `# DIAGNOSTIC RESULTS` sections. Config A headline: all 4 Phase-2 datasets pass hard bar with 5/5 per-seed matched-seed margin (CiteSeer Z_concat 71.88 vs raw best 64.76; PubMed Z_concat 80.77 vs 76.57; Photo Z_concat 93.91 vs 92.70; CS Z_mean 94.37 vs 92.96). Combined with INQ-007 this is 7/7 homophilic datasets passing. Config B: D6c vs SUGRL measured on Cora/Computers/arxiv with matched hardware. SUGRL Computers trial 5 was killed by initial 6h SLURM time limit (due to tqdm output buffering that made the first attempt appear hung); unbuffered rerun hit the same limit; arxiv was done via a dedicated arxiv-only resubmit that completed in 2:58. SUGRL VRAM not instrumented (only CPU RSS); flagged as measurement gap. Other baselines (BGRL/GraphMAE/PolyGCL/GGD/MHVGCL) not in repo — flagged not-run rather than half-reproduced. Configs C/D skipped per capacity tradeoff. Frontmatter open→answered; README index updated.

## [2026-04-24] inquiry-answer #2 | INQ-2026-04-24-001 matched-codebase rerun
CA built a matched harness (ad_ssl/experiments/adssl_sugrl.py) that runs SUGRL under the same ad_ssl shell as D6c — same load_dataset, seed_everything, timing pattern, torch.cuda.max_memory_allocated. SUGRL keeps its paper-faithful adjacency (sym-norm non-arxiv, row-norm arxiv), multiplet loss ported from train.py, and args.yaml probe hyperparameters; probe_restarts bumped 2→5 so n=25 matches D6c. Also added a --adj_norm {sym,row} flag to adssl_d6.py + new build_row_norm_adj in ad_ssl/precompute.py so D6c can be evaluated on arxiv under the same adjacency convention as SUGRL. Three SLURM jobs (D6c-5seed on all three, D6c-arxiv-rownorm, SUGRL-matched) completed on Vector A40. Headline numbers appended under `# DIAGNOSTIC RESULTS — Config B (matched-codebase rerun)`: D6c-Z_concat Cora 81.92 / Computers 87.93 / arxiv-sym 68.31 / arxiv-row 68.46; SUGRL Cora 81.74 / Computers 83.25 / arxiv 63.29. Matched n=25 each, matched hardware, matched timing and memory instrumentation. SUGRL-matched accuracy differs from the original train.py numbers because (a) 5 different seeds → 5 different Option B splits on Computers (vs fixed seed=0 × 5 trials in train.py), and (b) 5 probe restarts vs 2. Wiki log updated; submodule committed.

## [2026-04-24] RA analysis | INQ-008 independent read — 7/7 homophilic PASS; D6c beats SUGRL on matched harness
Independent read of [[INQ-2026-04-24-001]] Config A + B (initial + matched-codebase rerun). **Headline reframe needed: the Pareto story is stronger than "within the cheap-method band."** On matched harness (n=25, same splits, same hardware, same timing/memory instrumentation), D6c-Z_concat beats SUGRL by +0.30 on Cora (81.92 vs 81.74, overlapping CIs), **+4.86 on Computers** (87.93 vs 83.25), and **+5.17 on arxiv** (68.46 vs 63.29). Computers and arxiv deltas are large, CI-disjoint, and outside the "within cheap band" framing that [[Pareto Gap]] currently uses — D6c is not just matched-cost-at-similar-accuracy, it is **matched-cost-at-higher-accuracy** on two of three benchmarks relative to SUGRL. RA findings CA did not highlight: (1) **Photo cos(W_k, W_k') = 0.73–0.97** — per-depth projections nearly redundant on the easiest Phase-2 dataset, yet the dataset still passes; evidence that the residual floor does the work and per-depth parameterization is not load-bearing when all depths are already clean (mechanism, potential reviewer-attack surface: "why K W_k if they collapse?" → because Computers and Cora show cos as low as 0.04 where depths DO decouple). (2) **PubMed k=8 Z_k = 76.48 vs raw 77.12 (−0.64)** — one post-D6c depth actually drops below raw; PubMed is the first dataset where D6c does not uniformly lift per-depth, and the headline Z_concat +4.20 is carried by k=0,1,2,4. (3) **CS is the first dataset where Z_mean > Z_concat** (94.37 vs 94.31); readout choice is dataset-dependent, not globally Z_concat-best as INQ-007 arxiv suggested. (4) CiteSeer k=0 Z_k = 69.00 vs raw 46.50 (+22.50) — largest per-depth lift across all 7 datasets; the cross-depth InfoNCE pulls raw features (k=0) up into the shared simplex far more aggressively than any other (dataset, depth) combination. (5) **Missing-baseline blocker for Pareto figure**: Config B only ran D6c vs SUGRL. BGRL/GraphMAE/PolyGCL/GGD/MHVGCL are not in the ad_ssl repo so were flagged not-run. Without those, the Pareto figure can show (D6c, SUGRL) as a two-point cheap-method panel but cannot defend the "+8.05 vs encoder-methods at matched cost" reviewer claim. Blocker — needs follow-up inquiry (INQ-009) deciding between (a) port BGRL/GraphMAE/PolyGCL into the matched harness (expensive, slow), (b) cite paper numbers with hardware caveat (risk: reviewers push back on hardware mismatch), or (c) scope the efficiency claim to "vs. SUGRL" only and cite other baselines on accuracy-only axis. (6) **SUGRL-matched vs SUGRL-original gap** (Cora −1.49, Computers −5.26, arxiv −5.75) is a protocol artifact: different split seeds + 5 vs 2 probe restarts. Decision needed: paper reports SUGRL-matched (apples-to-apples, shows D6c advantage bigger) or SUGRL-original (weaker D6c advantage but less attackable on "did you cherry-pick the SUGRL protocol" grounds).

## [2026-04-24] inquiry-open | INQ-2026-04-24-002 D6c final experiments plan
Master experimental plan shipped to CA. Closes Phase 3, opens paper-writing phase. Scope: (A) Optuna HPO for D6c per-dataset; (B) efficiency benchmark (wall-clock + peak memory + FLOPs + precompute) on matched harness for D6c + BGRL + GGD + GraphMAE2 + GraphACL; (C) ablation studies (architecture A0–A5, loss B0–B3 including MHVGCL-defense B2 per-depth-independent InfoNCE, readout C0–C5, K-sweep D0–D5); (D) sensitivity around tuned optimum on 3 representative datasets; (E) mechanism diagnostics (per-depth lift, cos(W_k, W_k'), ||W_k||_F, alignment+uniformity, simplex-collapse per Ji et al. 2025, per-node depth preference). Baseline inventory locked at 13 main-table methods (classical: DGI/MVGRL/GRACE/CCA-SSG; modern: SUGRL/BGRL/GGD/GraphMAE2/GraphACL/PolyGCL/DGD/MHVGCL/BLNN). Port-selection criteria (a) recent ≥2022, (b) SOTA-tier accuracy, (c) explicit scalability, (d) public code, (e) no-OOM-on-A40-48GB — strict intersection yields BGRL/GGD/GraphMAE2/GraphACL. Split protocol: Planetoid public for Cora/CiteSeer/PubMed, 10/10/80 random for Amazon/CS, OGB `get_idx_split()` for arxiv. Fidelity: 3×3=9 during search, 5×5=25 for final headline. Posture A locked (our HPO for D6c; baselines cited as-reported). Dependencies: Phase 1 = Config A, Phase 2 = baseline ports, Phase 3 = Configs B/C/D/E.

## [2026-04-24] inquiry-answer | INQ-2026-04-24-002 D6c final experiments plan (execution plan ack)
CA acknowledged the master five-config plan (A: Optuna HPO, B: efficiency, C.1–C.4: ablations, D: sensitivity, E: mechanism). No infeasibility flags on the scientific spec. Execution structure: new working branch `ad-ssl/neurips-experiments` from main; `ad-ssl/d6c-phase2` frozen. Plan docs checked in at `docs/neurips-experiments/` on main — README, master-plan, vector-slurm-reference, plus 9 self-contained batch docs (one per runnable sub-experiment) so Jun (collaborator with Vector cluster access) can execute sbatch batches per doc while Radin owns harness, variant flags, port code, and RA-facing reporting. Operational choices: SQLite-backed Optuna per dataset with WAL + 30s busy-timeout; `scavenger` QoS for HPO search workers (unlimited GPUs, preempt-tolerant since Optuna resumes); `normal` QoS on `a40_b2` only for Config B (non-preempted, single GPU type across all cells for comparability); fvcore for FLOPs with total = 3× forward heuristic; port-validation criterion 5 pts on ≥2 datasets confirmed. Phases 1 and 2 parallelizable. Four open questions back to RA: (1) SQLite acceptable at >16 workers per study or move to Postgres/RDB? (2) confirm per-baseline validation datasets for BGRL/GGD/GraphMAE2/GraphACL? (3) fvcore vs alternative FLOPs tool? (4) is Config A SQLite DB a paper-submission artifact? Plus one clarification on C.3 readout overlap with Config A's search space. No results yet; per-phase `# RESULTS — Phase X` appendages will follow as batches complete.

## [2026-04-24] inquiry-answer #2 | INQ-2026-04-24-002 RA clarifications to CA
RA returned RESPONSE #2 to CA's five clarifying questions. (1) SQLite + WAL + 30s busy-timeout acceptable up to ~20 workers/study; escalate to Postgres only if `database is locked` rates exceed ~1% of trial completions. (2) Per-baseline port-validation pairings recommended (each ≤5 pts of paper number on one large + one small graph): BGRL = arxiv + Computers; GGD = arxiv + Cora; GraphMAE2 = arxiv + Cora; GraphACL = arxiv + Photo. Each baseline must hit at least one large-graph (arxiv) target so the port is validated at scale. Final dataset choice is CA's. (3) `fvcore.FlopCountAnalysis` approved with `flops_total ≈ 3 × flops_forward` heuristic documented in methods footnote; fall back to ptflops/torch.profiler per-method if fvcore chokes on a specific op. (4) Config A SQLite DB published as reproducibility artifact alongside released code (NeurIPS norm; defense against cherry-pick attacks). (5) Config C.3 readout sweep: re-run all 5 non-winner readouts × 7 datasets at Config A's tuned (τ, lr, WD, K_set, adj_norm) — net 875 new runs; do NOT reuse Config A's top-5 confirmation artifacts (ranking not aligned with readout family). Phase 1 (Config A HPO) and Phase 2 (baseline ports) unblocked; Phase 3 stays gated. No scope changes; all CA operational choices accepted. RESPONSE #2 committed and pushed (d699c96).
